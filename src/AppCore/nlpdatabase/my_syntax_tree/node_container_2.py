class NodeContainer:
    ...
    def add_props(self, proper):
        ...
        cypher = "(({}) -[{}*0..1]-> ({}) ".format(self.id, relkey, rg)
        proplst = []
        for i in prop:
            proplst.append("({}.{} in {})".format(self.id, i.system_name, str(slist)))
        if rels:
            proplst.append("(type({}) in {})".format(relkey, str([i.system_name for i in rels])))
    ...
    def get_cypher(self):
        cypher = []
        if self.is_a:
            cypher.append(self.__create_base())
        if self.data_properties_resolved:
            cypher.append(self.create_named_props())
        if self.data_relations:
            cypher.append(self.create_relationships())
        if self.data_properties:
            cypher.append(self.create_data_pops())
        res = "({})".format(str(cypher))
        return cypher
